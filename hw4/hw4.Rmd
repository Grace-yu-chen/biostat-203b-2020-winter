...
#vert_pos = rev(auto_coords[,15])
box_pos <- matrix(nrow = num_of_boxes, ncol = 2, data = 0)
box_pos[1,] = c(0.20, 0.1) # 1st box
box_pos[2,] = c(0.20, 0.2) # 2nd box
box_pos[3,] = c(0.20, 0.3) # 3rd box
box_pos[4,] = c(0.90, 0.4) # etc...
box_pos[5,] = c(0.90, 0.5)
box_pos[6,] = c(0.90, 0.6)
box_pos[7,] = c(0.90, 0.7)
box_pos[8,] = c(0.90, 0.8)
box_pos[9,] = c(0.20, 0.9)
box_pos[10,] = c(0.20, 1)
box_pos[11,] = c(0.90, 1.1)
box_pos[12,] = c(0.90, 1.2)
box_pos[13,] = c(0.90, 1.3)
box_pos[14,] = c(0.90, 1.4)

# content of boxes
box_content <- matrix(nrow = num_of_boxes, ncol = 1, data = 0)
box_content[1] = "Patients admitted to all care units/n CCU,CSRU,MICU,NICU,NWARD,SICU and TSICU" # 1st box
box_content[2] = "Patients diagnosed with Myocardia Infarction" # 2nd box
box_content[3] = "Indicating the MI is the principal diagonosis or not" # 3rd box
box_content[4] = "Add the severity of patients’ ailments" # etc...
box_content[5] = "Keep information of patients who didn't die in hospital"
box_content[6] = "Add age, exclude patients over 90 years old"
box_content[7] = "Add Ethnicity and Gender"
box_content[8] = "Calculate death rate and population"
box_content[9] = "Detailed analysis of patients admitted to CCU and SICU"
box_content[10] = "Age"
box_content[11] = "Severty"
box_content[12] = "Gender"
box_content[13] = "Ethnicity"
box_content[14] = "Conclusion"

# adjust the size of boxes to fit content
box_x <- c(0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25)
box_y <- c(0.07, 0.07, 0.07, 0.07, 0.07, 0.07, 0.07, 0.07, 0.07, 0.07, 0.07, 0.07, 0.07, 0.07)

# Draw the arrows
straightarrow(from = box_pos[1,], to = box_pos[2,], lwd = 1)
straightarrow(from = c(box_pos[2,1],box_pos[3,2]), to = box_pos[3,], lwd = 1) 
straightarrow(from = c(box_pos[2,1],box_pos[4,2]), to = box_pos[4,], lwd = 1)  
straightarrow(from = c(box_pos[2,1],box_pos[5,2]), to = box_pos[5,], lwd = 1) 
straightarrow(from = c(box_pos[2,1],box_pos[6,2]), to = box_pos[6,], lwd = 1)
straightarrow(from = c(box_pos[2,1],box_pos[7,2]), to = box_pos[7,], lwd = 1)
straightarrow(from = box_pos[2,], to = box_pos[8,], lwd = 1)
straightarrow(from = box_pos[8,], to = box_pos[9,], lwd = 1)
straightarrow(from = c(box_pos[9,1],box_pos[10,2]), to = box_pos[10,], lwd = 1)
straightarrow(from = c(box_pos[9,1],box_pos[11,2]), to = box_pos[11,], lwd = 1)
straightarrow(from = c(box_pos[9,1],box_pos[12,2]), to = box_pos[12,], lwd = 1)
straightarrow(from = c(box_pos[9,1],box_pos[13,2]), to = box_pos[13,], lwd = 1)
straightarrow(from = box_pos[9,], to = box_pos[14,], lwd = 1)
# Draw the boxes
